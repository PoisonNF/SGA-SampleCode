...
uint16_t Drv_Uart_Receive_DMA(tagUART_T *_tUART, uint8_t *_ucpRxData)
{
	...
	if(_tUART->tUartDMA.ucDMARxCplt != 1)
	{
		return 0;
	}
	else
	{
		usRxNum = _tUART->tRxInfo.usRxLenth;
        memset(_ucpRxData,0,usRxNum+1);
		memcpy(_ucpRxData,_tUART->tRxInfo.ucpRxCache,usRxNum);
		_tUART->tUartDMA.ucDMARxCplt = 0;
		return usRxNum;
	}
}
